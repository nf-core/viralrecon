...
def helpMessage() {
    ...
    References                        If not specified in the configuration file or you wish to overwrite any of the references
      --genome [str]                  Name of genome reference key for viral genome (Default: 'NC_045512.2')
    ...
    fastq file download
      --ncbi_api_key [str]             NCBI API key increases the number of requests to NCBI SRA database (https://ncbiinsights.ncbi.nlm.nih.gov/2017/11/02/new-api-keys-for-the-e-utilities/)
 ...
 */
process VALIDATE_FASTQ_NF_FROMSRA {
    tag "$id"
    label 'process_low'
    input:
...
// ch_fasterq_gz_validated_to_print.view()
...
process SPADES_QUAST {
    ...
    script:
    gffpath = params.gff ? "GFF=$gff; if [[ \$GFF == *.gz ]]; then gunzip -f \$GFF; GFF=\${GFF%.*}; fi" : ""
...
process MULTIQC {
    ...
    file ('trimmomatic/amplicon/*') from ch_trimmomatic_amplicon_mqc.collect().ifEmpty([])
