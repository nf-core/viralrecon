...
* Supply your own via the `--nextclade_dataset` parameter
* Let the pipeline create and use the latest version by setting `--nextclade_dataset false --nextclade_dataset_tag false`
* Let the pipeline create and use a more recent tagged version by setting `--nextclade_dataset false --nextclade_dataset_tag <DATASET_TAG>`

If the `--save_reference` parameter is provided then the Nextclade dataset downloaded by the pipeline will be saved in the `results/genome/` directory.
