...
- [Nanopore: Pipeline overview](#nanopore-pipeline-overview)
  ...
  - [Nanopore: Mapping, Variant calling and consensus](#nanopore-mapping-variant-calling-and-consensus)
    - [Nanopore: ARTIC](#nanopore-artic)
    - [Nanopore: minimap2, Clair3 and bcftools](#nanopore-minimap2-clair3-and-bcftools)
...
## Nanopore: Mapping, Variant calling and consensus

The output directory depends on the value provided to `--mapper_nanopore`:

- `artic/`: generated when using `--mapper_nanopore artic`. ARTIC performs mapping, variant calling and consensus generation.
- `minimap2/`: generated when using `--mapper_nanopore minimap2`. Minimap2 performs mapping, Clair3 performs variant calling and BCFtools generates the consensus.
...
- For `--mapper_nanopore 'artic'`: `artic/`
  ...
  - `*.merged.vcf.gz`: Compressed VCF file containing all detected variants.
  - `*.merged.vcf.gz.tbi`: Index file for compressed merged VCF.
  - `*.normalised.vcf.gz`: Normalised VCF file.
  - `*.normalised.vcf.gz.tbi`: Index file for normalised VCF.
  ...
  - `*.1.trimmed.rg.sorted.bam` and `*.2.trimmed.rg.sorted.bam`: Per-read-group trimmed BAM files.
  - `*.1.trimmed.rg.sorted.bam.bai` and `*.2.trimmed.rg.sorted.bam.bai`: Per-read-group trimmed BAM index files.
...
### Nanopore: minimap2, Clair3 and bcftools

When `--mapper_nanopore minimap2` is selected, the pipeline uses [minimap2](https://github.com/lh3/minimap2) to align Nanopore reads to the reference genome. Minimap2 is a fast sequence aligner designed for long-read data and is widely used for Oxford Nanopore and PacBio mapping.

Variants are then called with [Clair3](https://github.com/HKU-BAL/Clair3), a deep-learning-based small variant caller for long-read sequencing data. Clair3 requires an appropriate model for the sequencing chemistry and basecaller used; these can be provided with the `--clair3_model_dir` and `--clair3_model` parameters.

The final consensus sequence is generated with [BCFtools](https://github.com/samtools/bcftools), using the filtered and normalised Clair3 variant calls. BCFtools provides utilities for manipulating VCF/BCF files and generating consensus FASTA sequences from a reference genome and a set of variants.
...
- `minimap2/`
  ...
  - `minimap2/bcftools/`
    ...
    - `*.consensus.vcf.gz.tbi`: Index file for consensus VCF.
...
**NB:** The value of `<MAPPER>` in the output directory name above is determined by `--mapper_nanopore` (`artic` or `minimap2`).

</details>

BAM files containing the original alignments from either ARTIC or Minimap2 are further processed with [SAMtools](http://samtools.sourceforge.net/) to remove unmapped reads as well as to generate read mapping statistics.
...
- `<MAPPER>/freyja/SARS-CoV-2`
  - `curated_lineages.json`: Dataset containing lineage metadata that correspond to barcodes.
  - `lineages.yml`: Dataset containing the lineage topology.
  - `pathogen_config.yml`: Dataset configuration file.
  - `usher_barcodes.feather`: Dataset containing lineage defining barcodes.
- `<MAPPER>/freyja/variants`
...
- `variants/freyja/bootstrap`
  - `*lineages.csv` Analysis results including lineages present and their corresponding abundances with variation identified through bootstrapping
  - `*summarized.csv`Analysis results including lineages present but summarized by constellation and their corresponding abundances with variation identified through bootstrapping
...
- `assembly/minia/`
  ...
  - `*.contigs.fa.gz`: Compressed Minia scaffold assembly.
  - `*.unitigs.fa.gz`: Minia unitigs fasta file.
...
- `assembly/<ASSEMBLER>/abacas/`
  ...
  - `*.abacas.unused.contigs.out`: Information on contigs that have mapping information but could not be used in the ordering.
...
- `variants/<CALLER>/consensus/<CONSENSUS_CALLER>/resistance_reports`
  ...
  - `resistance.html`: HTML report with resistance and mutation results. It was meant to be as similar as possible as the ones generated by [HIVdb](https://hivdb.stanford.edu/hivdb/by-reads).

Parameters used by `ivar variants` and `ivar consensus` are reported in the sequence summary box.

If you override these settings, please keep them synchronized with the corresponding `iVar` process parameters.
