...
* `assembly/<ASSEMBLER>/abacas/`
  ...
  * `*.abacas.gaps.tab`: Gap information in tab format.
...
* `assembly/<ASSEMBLER>/abacas/nucmer/`: Folder containing the files generated by the Nucmer algorithm used by ABACAS to
...
* `assembly/<ASSEMBLER>/plasmidid/<SAMPLE>/`
  ...
  * `fasta_files`: Folder with fasta files. These fasta files correspond to the selection of those contigs/scaffolds that reconstruct the reference genome showed in images folder.
  * `log/`: Log files.
